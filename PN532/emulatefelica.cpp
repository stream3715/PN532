...
bool EmulateFelica::emulate(const uint16_t tgInitAsTargetTimeout)
{
  ...
  while (runLoop)
  {
    status = pn532.tgGetInitiatorCommand(rwbuf, sizeof(rwbuf));
    if (status < 0)
    {
      DMSG("tgGetInitiatorCommand failed!\n");
      pn532.inRelease();
      return true;
    }
    ...
    */
    ...
    status = pn532.tgResponseToInitiator(rwbuf, sendlen);
    if (status < 0)
    {
